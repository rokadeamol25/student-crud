    ...
    if (req.method === 'PATCH') {
      ...
      if (rawItems !== null) {
        ...
        const { data: invoice, error: updateErr } = await supabase
          .from('invoices')
          .update({ customer_id: customerId, invoice_date: invoiceDate, subtotal, tax_percent: effectiveTaxPercent, tax_amount: taxAmount, discount_total: discountTotal, total, gst_type: gstType, rough_bill_ref: roughBillRef })
          .eq('id', id)
          .eq('tenant_id', tenantId)
          .select()
          .single();
        ...
        const itemRow = (it, includeCost) => {
          ...
          if (it.discount_type) row.discount_type = it.discount_type;
          if (it.discount_value != null) row.discount_value = it.discount_value;
          if (it.discount_amount != null) row.discount_amount = it.discount_amount;
          return row;
        };
