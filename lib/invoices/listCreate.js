...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const orderDir = (req.query?.order ?? 'desc').toString().toLowerCase() === 'asc';
      let q = supabase.from('invoices').select('*', { count: 'exact' }).eq('tenant_id', tenantId).order(orderCol, { ascending: orderDir });
