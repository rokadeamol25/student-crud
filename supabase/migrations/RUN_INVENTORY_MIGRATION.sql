-- ============================================================
-- Inventory & tracking type migration (run this script once)
-- Combines 00014_product_tracking_type + 00015_inventory_tables
-- ============================================================

-- 1. Add new columns to products (safe to re-run: IF NOT EXISTS)
...
-- Set defaults for existing rows (in case columns were added without default)
UPDATE products SET tracking_type = 'quantity' WHERE tracking_type IS NULL;
UPDATE products SET is_active = TRUE WHERE is_active IS NULL;

-- Now enforce NOT NULL and default for new rows
ALTER TABLE products ALTER COLUMN tracking_type SET DEFAULT 'quantity';
...
-- Drop constraint if it exists (e.g. from a previous partial run), then add
ALTER TABLE products DROP CONSTRAINT IF EXISTS products_tracking_type_check;
ALTER TABLE products
